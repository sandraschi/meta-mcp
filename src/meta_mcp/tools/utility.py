...
class TextProcessingInput(BaseModel):
    ...
    max_length: Optional[int] = Field(
        None, description="Maximum length of the output text"
    )
...
@structured_log()
def format_text(
    text: str, variables: Dict[str, Any], template_syntax: str = "mustache"
) -> str:
    """
...
def validate_json(
    ...
    """
    result = {"valid": False, "errors": [], "normalized_data": None}
...
async def make_http_request(
    ...
    try:
        ...
        logger.error(
            "http_request_failed",
            extra={"method": method, "url": url, "error": str(e), "duration": duration},
            exc_info=True,
        )
        raise
...
async def schedule_task(
    func: callable, delay: float = 0.0, *args: Any, **kwargs: Any
) -> str:
    """
    ...
    async def wrapped():
        try:
            ...
            logger.error(
                ...
                extra={"task_id": task_id, "function": func.__name__, "error": str(e)},
                exc_info=True,
            )
            raise
