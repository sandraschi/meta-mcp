...
class ProfilingResult(BaseModel):
    ...
    per_call_times: Dict[str, float] = Field(
        ..., description="Time per function call in seconds"
    )
...
async def profile_code(
    ...
    if output_file:
        try:
            ...
            logger.warning(
                "failed_to_save_profile", extra={"path": output_file, "error": str(e)}
            )
...
@contextmanager
def trace_execution(func_name: str = "", log_level: str = "DEBUG", max_depth: int = 10):
    """
...
@contextmanager
def measure_memory(label: str = "", detailed: bool = False):
    """
