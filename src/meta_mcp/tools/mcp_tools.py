...
from .discovery import ToolRegistry, discover_tools, discover_servers
from .server_builder import create_mcp_server
from .runt_analyzer import analyze_runts
from .server_updater import update_mcp_server
from .files import FileInfo, list_directory, read_file, write_file
from .utility import generate_id, format_text, validate_json
...
def register_discovery_tools(app: FastMCP):
    ...
    async def discover_mcp_servers(
        ...
        try:
            # Use existing discovery functionality
            ...
            for search_path in search_paths:
                try:
                    ...
                    logger.warning("Failed to scan path", path=search_path, error=str(e))
            
            # Analyze discovered servers
            active_servers = [s for s in discovered_servers if s.get("status") == "active"]
            inactive_servers = [s for s in discovered_servers if s.get("status") != "active"]
            ...
            return {
                ...
                "result": {
                    ...
                    "servers": discovered_servers if detailed_scan else [
                        {"name": s.get("name", "Unknown"), "status": s.get("status", "unknown")}
    ...
    async def get_server_info(
        ...
        try:
            ...
            if not server_path:
                ...
                search_paths = ["d:\\Dev\\repos", "C:\\Users\\*\\AppData\\Roaming\\Claude"]
                server_path = None
...
def register_all_meta_tools(app: FastMCP):
    ...
    logger.info("MetaMCP tools registration complete - Enhanced response patterns enabled")
